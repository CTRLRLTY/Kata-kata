...
func render(graph_node: GDCharacterJoinGN, dialogue_view: GDDialogueView, cursor: GDDialogueCursor) -> void:
	if dialogue_view.has_method("set_character_left_texture") and \
	   dialogue_view.has_method("set_character_right_texture") and \
	   dialogue_view.has_method("character_join")\
	:
		...
		if expression_data:
			...
			dialogue_view.call("set_character_%s_texture" % [position], expression_data.expression_texture)
